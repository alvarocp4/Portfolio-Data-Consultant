...
import 'kleur/colors';
import { b as $$Footer, a as $$Header, $ as $$BaseHead } from '../chunks/Footer_CD2_mk5g.mjs';
import { S as SITE_DESCRIPTION, a as SITE_TITLE } from '../chunks/consts__lf-bKLc.mjs';
import { a as $$Projects, $ as $$HeroSection } from '../chunks/HeroSection_mze4JbCs.mjs';
import 'clsx';
/* empty css                                 */
...
const $$Timeline = createComponent(($$result, $$props, $$slots) => {
  ...
  const calendarIcon = props.icons?.calendar ?? normalizeIcon(calendarIconRaw);
  ...
  return renderTemplate`${centered ? renderTemplate`${maybeRenderHead()}<div class="relative mx-auto max-w-5xl px-0 md:px-0"><!-- Center vertical line --><div class="pointer-events-none absolute inset-y-0 left-1/2 w-px -translate-x-1/2 bg-[var(--border)]"></div><div class="space-y-12">${items.map((item, index) => {
    ...
    ], "class:list")}><div class="card-surface p-6"><div class="mb-2 space-y-1"><h3 class="text-xs leading-snug font-semibold m-0 flex items-center gap-2"><span class="inline-flex" aria-hidden="true">${unescapeHTML(jobIcon)}</span>${item.title}</h3><p class="text-sm font-medium text-[color:var(--color-text-muted)] m-0 flex items-center gap-2"><span class="inline-flex" aria-hidden="true">${unescapeHTML(calendarIcon)}</span>${item.period}</p><p class="text-sm font-medium text-[color:var(--color-text-muted)] m-0 flex items-center gap-2"><span class="inline-flex" aria-hidden="true">${unescapeHTML(companyIcon)}</span>${item.company}</p><p class="text-sm text-[color:var(--color-text-muted)] m-0 flex items-center gap-2"><span class="inline-flex" aria-hidden="true">${unescapeHTML(locationIcon)}</span>${item.region}</p><p class="text-sm text-[color:var(--color-text-muted)] m-0 flex items-center gap-2">${item.industry}</p></div><ul class="text-[color:var(--color-text-secondary)] m-0"><li>${item.description[0]}</li><li>${item.description[1]}</li><li>${item.description[2]}</li><li>${item.description[3]}</li><li>${item.description[4]}</li></ul>${item.technologies && item.technologies.length > 0 && renderTemplate`<div class="mt-3 flex flex-wrap gap-2">${item.technologies.map((tech) => renderTemplate`<span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-medium" style="background: rgba(var(--accent-rgb),0.1); color: var(--accent); border: 1px solid rgba(var(--accent-rgb),0.3);">${tech}</span>`)}</div>`}</div></div></div>`;
